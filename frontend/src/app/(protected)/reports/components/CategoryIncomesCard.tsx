...
export type IncomeCategoryReportRow = {
  category_id: number;
...
/**
 * CategoryIncomesCard (responsive + Top/Worst single-select)
 * - Same UX as ProductSalesCard
 * - Filters: date range + multi-category
 * - Chart: amount + count (two bars)
 */
export function CategoryIncomesCard() {
  ...
  const [rows, setRows] = React.useState<IncomeCategoryReportRow[]>([]);
  ...
  const fetchReport = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await api.post<SuccessResponse<IncomeCategoryReportRow[]>>('/reports/incomes', payload, {
        headers: { 'Content-Type': 'application/json' },
      });
      const data = (res.data as any).data ?? res.data;
      const list: IncomeCategoryReportRow[] = Array.isArray(data) ? data : [];
      setRows(list);
              ...
              <div className="grid gap-1 min-w-0">
                ...
                <Select value={topWorst} onValueChange={(v: TWMode) => setTopWorst(v)}>
                  <SelectTrigger className="min-w-0 w-full">
                    ...
                    <SelectContent>
                      <SelectItem value="none">Nessun filtro</SelectItem>
                      <SelectItem value="top5">5 più redditizie (totale €)</SelectItem>
                      <SelectItem value="top10">10 più redditizie (totale €)</SelectItem>
                      <SelectItem value="worst5">5 meno redditizie (totale €)</SelectItem>
                      <SelectItem value="worst10">10 meno redditizie (totale €)</SelectItem>
                    </SelectContent>
                </Select>
              </div>
            </div>
