...
export default function IncomesCard() {
  ...
  // ===============================
  // Fetch categories via POST /incomes/categories/list (size = -1)
  // ===============================
  ...
  async function bulkDeletePerform() {
    ...
    const results = await Promise.allSettled(
      selectedIds.map((id) => api.delete(`/incomes/${id}`))
    );
  ...
  return (
    <Card className="max-w-full">
      <CardHeader className="space-y-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          {selectedIds.length > 0 ? (
            ...
            <Button onClick={() => setAddOpen(true)}>+ Aggiungi entrata</Button>
          )}
        </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-2 min-w-0">
                        <Checkbox
                          ...
                          aria-label={`Seleziona entrata ${r.id}`}
      ...
      <AlertDialog open={confirmBulkOpen} onOpenChange={handleConfirmBulkOpenChange}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Eliminare {selectedIds.length} entrata/e?</AlertDialogTitle>
